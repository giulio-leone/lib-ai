...
import { createEnvVar, getEnvVarByKey, updateEnvVar, envVarExists, } from '@onecoach/lib-vercel-admin/vercel-env-vars-api.service';
...
export class AIProviderConfigService {
    ...
    static async listConfigs() {
        ...
        return Promise.all(providerEntries.map(async ([providerName, mapping]) => {
            ...
            return this.toDTO(storedConfig?.provider ?? mapping.enum, {
    ...
    static async syncToVercel(provider, apiKey, environments = ['production', 'preview']) {
        ...
        try {
            ...
            const result = existingResult.success && existingResult.data
                ? await updateEnvVar({
    ...
    static async upsertConfig(params) {
        ...
        const metadataToSave = this.buildMetadata(Object.keys(existingMetadata).length > 0 ? existingMetadata : null, {
        ...
        return this.toDTO(updated.provider, {
    ...
    static toDTO(provider, data, defaultModel) {
        ...
        const defaultProvider = providerName === 'openrouter'
            ? this.extractDefaultProvider(data.metadata)
            : null;
    ...
    static buildMetadata(current, params) {
        ...
        const existingHistory = Array.isArray(historyValue)
            ? historyValue
            : [];
