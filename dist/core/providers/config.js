...
export async function getModelByTier(tier = 'balanced') {
    ...
    if (!adminConfig) {
        throw new Error(`[ModelConfig] No default model configured in admin dashboard. Please configure a default model in /admin/ai-settings.`);
    }
    console.warn(`[ModelConfig] Using admin-configured model for ${tier}: ${adminConfig.provider}/${adminConfig.modelId}`);
...
export function getModelsByTiers(tiers, minModels = 2, maxModels = 3) {
    ...
    if (selectedModels.length < minModels) {
        ...
        for (const config of fallback) {
            const modelId = Object.keys(MODEL_CONFIGS).find((id) => MODEL_CONFIGS[id]?.model === config.model);
