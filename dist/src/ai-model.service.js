...
export class AIModelService {
    ...
    static async getStandardizedModelConfig(operationType, requestedModel, logger) {
        ...
        if (!modelId) {
            throw new Error("Nessun modello AI configurato! Contatta l'amministratore per impostare un modello predefinito.");
        }
        ...
        if (providerName === 'gemini-cli') {
            ...
            const createProvider = ProviderFactory.createProviderInstance(providerName, apiKey, preferredProvider);
            model = createProvider(modelId);
        }
        // Calculate effective max output tokens
        const effectiveMaxTokens = Math.min(maxTokens ?? TOKEN_LIMITS.DEFAULT_MAX_TOKENS, TOKEN_LIMITS.MAX_OUTPUT);
    ...
    static determineProvider(modelId) {
        ...
        if (lowerId.startsWith('minimax-') ||
            lowerId === 'minimax-m2.1' ||
            lowerId.includes('minimax')) {
            return 'minimax';
        }
        if (lowerId.startsWith('claude-') || lowerId.includes('claude'))
            return 'anthropic';
        ...
        if (lowerId.startsWith('gemini-') || lowerId.includes('gemini'))
            return 'google';
        if (lowerId.startsWith('grok-') || lowerId.includes('grok'))
            return 'xai';
        // Default fallback
        return 'openrouter';
    }
    static async getApiKeyForProvider(provider) {
        try {
            ...
            const { AIProviderConfigService } = await import('./core/providers/ai-provider-config.service');
