 ...
 */
export async function getModelByTier(tier: ModelTier = 'balanced'): Promise<ModelConfig> {
 ...
 */
export function getFallbackChain(): ModelConfig[] {
  return [
    ...
    MODEL_CONFIGS['grok-4-fast']!, // Last resort: xAI direct
  ];
}
 ...
 */
export function getModelsByTiers(
  tiers: Array<'fast' | 'balanced' | 'quality'>,
  minModels: number = 2,
  maxModels: number = 3
): ExtendedModelConfig[] {
  const tierModels: Record<'fast' | 'balanced' | 'quality', string[]> = {
  ...
  const selectedModels: ExtendedModelConfig[] = [];
  ...
  if (selectedModels.length < minModels) {
    ...
    for (const config of fallback) {
      const modelId = Object.keys(MODEL_CONFIGS).find(
        (id: string) => MODEL_CONFIGS[id]?.model === config.model
      );
